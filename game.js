...
leftButton.addEventListener("touchstart", () => moveBasketDirection("left"));
rightButton.addEventListener("touchstart", () => moveBasketDirection("right"));

leftButton.addEventListener("click", () => moveBasketDirection("left"));
rightButton.addEventListener("click", () => moveBasketDirection("right"));

// Fungsi untuk menggerakkan keranjang
function moveBasketDirection(direction) {
    if (gameState !== "play") return;

    const moveDistance = 65; // Jarak gerakan keranjang
    if (direction === "left" && basketX > 0) {
        basketX -= moveDistance;
    } else if (direction === "right" && basketX < canvas.width - basketWidth) {
        basketX += moveDistance;
    }
}

// Perbarui posisi keranjang setiap frame
function drawBasket() {
